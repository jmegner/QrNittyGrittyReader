    ...
    while (stream.available() >= 4) {
        ...
        if (mode === ModeByte.Terminator) {
            ...
            if (stream.readBits(1) === 0) {
                ...
                assignmentNumber = -1; // corrupted ECI
            }
            var after = stream.available();
            var bodyBits = before - after;
            result.chunks.push({
                type: Mode.ECI,
            ...
            var after = stream.available();
            var bodyBits = before - after;
            var ccBits = [10, 12, 14][size];
            result.text += numericResult.text;
            (_a = result.bytes).push.apply(_a, numericResult.bytes);
            result.chunks.push({
                type: Mode.Numeric,
                text: numericResult.text,
                characterCount: numericResult.text.length,
                modeIndicatorBits: 4,
            ...
            var after = stream.available();
            var bodyBits = before - after;
            var ccBits = [9, 11, 13][size];
            result.text += alphanumericResult.text;
            (_b = result.bytes).push.apply(_b, alphanumericResult.bytes);
            result.chunks.push({
                type: Mode.Alphanumeric,
                text: alphanumericResult.text,
                characterCount: alphanumericResult.text.length,
                modeIndicatorBits: 4,
                characterCountBits: ccBits,
                dataBits: bodyBits - ccBits,
                totalBits: 4 + bodyBits,
            ...
            result.chunks.push({
                ...
                byteCount: byteResult.bytes.length,
                encoding: "UTF-8",
                modeIndicatorBits: 4,
                characterCountBits: ccBits,
                dataBits: bodyBits - ccBits,
            ...
            result.chunks.push({
                ...
                characterCount: kanjiResult.text.length,
                modeIndicatorBits: 4,
                characterCountBits: ccBits,
                dataBits: bodyBits - ccBits,
                totalBits: 4 + bodyBits,
